...
namespace Core::Memory {
  ...
  namespace {
    ...
    bool AddressSpaceContains(const Common::PageTable & table,
      const Common::ProcessAddress addr,
        const std::size_t size) {
      const Common::ProcessAddress max_addr = 1 ULL << table.GetAddressSpaceBits();
  ...
  struct Memory::Impl {
    explicit Impl(Core::System & system_): system {
      system_
    } {}

    namespace Core::Memory {

      bool Memory::Impl::IsValidAddress(const Common::ProcessAddress addr, size_t size) const {
        // Check if the address is within the valid address space
        const Common::ProcessAddress max_addr = 1 ULL << current_page_table -> GetAddressSpaceBits();
        if (addr + size < addr || addr + size > max_addr) {
          LOG_ERROR(HW_Memory, "Invalid memory address: {:016X}, size: {:X}", addr, size);
          return false;
        }
        return true;
      }

      void Memory::Impl::Write8(const Common::ProcessAddress addr,
        const u8 data) {
        if (!IsValidAddress(addr, sizeof(u8))) {
          LOG_ERROR(HW_Memory, "Unmapped Write8 @ {:016X} = {:02X}", addr, data);
          return;
        }
        Write < u8 > (addr, data);
      }

      void Memory::Impl::Write16(const Common::ProcessAddress addr,
        const u16 data) {
        if (!IsValidAddress(addr, sizeof(u16))) {
          LOG_ERROR(HW_Memory, "Unmapped Write16 @ {:016X} = {:04X}", addr, data);
          return;
        }
        Write < u16_le > (addr, data);
      }

      void Memory::Impl::Write32(const Common::ProcessAddress addr,
        const u32 data) {
        if (!IsValidAddress(addr, sizeof(u32))) {
          LOG_ERROR(HW_Memory, "Unmapped Write32 @ {:016X} = {:08X}", addr, data);
          return;
        }
        Write < u32_le > (addr, data);
      }

      void Memory::Impl::Write64(const Common::ProcessAddress addr,
        const u64 data) {
        if (!IsValidAddress(addr, sizeof(u64))) {
          LOG_ERROR(HW_Memory, "Unmapped Write64 @ {:016X} = {:016X}", addr, data);
          return;
        }
  ...
  void MapMemoryRegion(Common::PageTable & page_table, Common::ProcessAddress base, u64 size,
    ...
    bool separate_heap) {
    ASSERT_MSG((size & UZUY_PAGEMASK) == 0, "non-page aligned size: {:016X}", size);
    ASSERT_MSG((base & UZUY_PAGEMASK) == 0, "non-page aligned base: {:016X}", GetInteger(base));
    ASSERT_MSG(target >= DramMemoryMap::Base, "Out of bounds target: {:016X}",
      GetInteger(target));
    MapPages(page_table, base / UZUY_PAGESIZE, size / UZUY_PAGESIZE, target,
  ...
  void UnmapRegion(Common::PageTable & page_table, Common::ProcessAddress base, u64 size,
    ...
    ASSERT_MSG((size & UZUY_PAGEMASK) == 0, "non-page aligned size: {:016X}", size);
    ASSERT_MSG((base & UZUY_PAGEMASK) == 0, "non-page aligned base: {:016X}", GetInteger(base));
    MapPages(page_table, base / UZUY_PAGESIZE, size / UZUY_PAGESIZE, 0,
  ...
  void ProtectRegion(Common::PageTable & page_table, VAddr vaddr, u64 size,
    ...
    ASSERT_MSG((size & UZUY_PAGEMASK) == 0, "non-page aligned size: {:016X}", size);
    ASSERT_MSG((vaddr & UZUY_PAGEMASK) == 0, "non-page aligned base: {:016X}", vaddr);
    ...
    u64 protect_bytes {};
    u64 protect_begin {};
    for (u64 addr = vaddr; addr < vaddr + size; addr += UZUY_PAGESIZE) {
      const Common::PageType page_type {
        current_page_table -> pointers[addr >> UZUY_PAGEBITS].Type()
      };
      switch (page_type) {
  ...
  [[nodiscard]] u8 * GetPointerFromRasterizerCachedMemory(u64 vaddr) const {
    const Common::PhysicalAddress paddr {
      current_page_table -> backing_addr[vaddr >> UZUY_PAGEBITS]
    };
  ...
  u64 Read64(const Common::ProcessAddress addr) {
    if ((addr & 7) == 0) {
      ...
      const u32 b {
        Read32(addr + sizeof(u32))
      };
      return (static_cast < u64 > (b) << 32) | a;
    }
  }

  void Write8(const Common::ProcessAddress addr,
    const u8 data) {
    Write < u8 > (addr, data);
  }

  void Write16(const Common::ProcessAddress addr,
    const u16 data) {
    if ((addr & 1) == 0) {
      Write < u16_le > (addr, data);
    } else {
  ...
  void Write32(const Common::ProcessAddress addr,
    const u32 data) {
    if ((addr & 3) == 0) {
      Write < u32_le > (addr, data);
    } else {
  ...
  void Write64(const Common::ProcessAddress addr,
    const u64 data) {
    if ((addr & 7) == 0) {
      Write < u64_le > (addr, data);
    } else {
  ...
  bool WriteExclusive8(const Common::ProcessAddress addr,
    const u8 data,
      const u8 expected) {
    return WriteExclusive < u8 > (addr, data, expected);
  }

  bool WriteExclusive16(const Common::ProcessAddress addr,
    const u16 data,
      const u16 expected) {
    return WriteExclusive < u16_le > (addr, data, expected);
  }

  bool WriteExclusive32(const Common::ProcessAddress addr,
    const u32 data,
      const u32 expected) {
    return WriteExclusive < u32_le > (addr, data, expected);
  }

  bool WriteExclusive64(const Common::ProcessAddress addr,
    const u64 data,
      const u64 expected) {
  ...
  bool WalkBlock(const Common::ProcessAddress addr,
    const std::size_t size, auto on_unmapped,
      auto on_memory, auto on_rasterizer, auto increment) {
    const auto & page_table = * current_page_table;
    std::size_t remaining_size = size;
    std::size_t page_index = addr >> UZUY_PAGEBITS;
    std::size_t page_offset = addr & UZUY_PAGEMASK;
    ...
    while (remaining_size) {
      const std::size_t copy_amount =
        std::min(static_cast < std::size_t > (UZUY_PAGESIZE) - page_offset, remaining_size);
      const auto current_vaddr =
        static_cast < u64 > ((page_index << UZUY_PAGEBITS) + page_offset);
      ...
      case Common::PageType::Memory: {
        u8 * mem_ptr =
          reinterpret_cast < u8 * > (pointer + page_offset + (page_index << UZUY_PAGEBITS));
      ...
      case Common::PageType::DebugMemory: {
        u8 *
          const mem_ptr {
            GetPointerFromDebugMemory(current_vaddr)
          };
        on_memory(copy_amount, mem_ptr);
        break;
      }
      case Common::PageType::RasterizerCachedMemory: {
        u8 *
          const host_ptr {
            GetPointerFromRasterizerCachedMemory(current_vaddr)
          };
  ...
  const u8 * GetSpan(const VAddr src_addr,
    const std::size_t size) const {
    if (current_page_table -> blocks[src_addr >> UZUY_PAGEBITS] ==
      current_page_table -> blocks[(src_addr + size) >> UZUY_PAGEBITS]) {
      return GetPointerSilent(src_addr);
    }
    return nullptr;
  }

  u8 * GetSpan(const VAddr src_addr,
    const std::size_t size) {
    if (current_page_table -> blocks[src_addr >> UZUY_PAGEBITS] ==
      current_page_table -> blocks[(src_addr + size) >> UZUY_PAGEBITS]) {
      return GetPointerSilent(src_addr);
  ...
  template < bool UNSAFE >
    bool WriteBlockImpl(const Common::ProcessAddress dest_addr,
      ...
      return WalkBlock(
        ...
        [ & ](const std::size_t copy_amount, u8 *
          const dest_ptr) {
        ...
        [ & ](const std::size_t copy_amount) {
          src_buffer = static_cast <
            const u8 * > (src_buffer) + copy_amount;
        });
    }

  bool WriteBlock(const Common::ProcessAddress dest_addr,
    const void * src_buffer,
      const std::size_t size) {
    return WriteBlockImpl < false > (dest_addr, src_buffer, size);
  }

  bool WriteBlockUnsafe(const Common::ProcessAddress dest_addr,
    const void * src_buffer,
      const std::size_t size) {
    return WriteBlockImpl < true > (dest_addr, src_buffer, size);
  }

  bool ZeroBlock(const Common::ProcessAddress dest_addr,
    const std::size_t size) {
    return WalkBlock(
      dest_addr, size,
      [dest_addr, size](const std::size_t copy_amount,
    ...
    Result PerformCacheOperation(Common::ProcessAddress dest_addr, std::size_t size,
      ...
      try {
        WalkBlock(
          ...
          [ & ](const std::size_t block_size, u8 *
            const host_ptr) {},
          [ & ](const Common::ProcessAddress current_vaddr,
            const std::size_t block_size,
              u8 *
  ...
  void MarkRegionDebug(u64 vaddr, u64 size, bool debug) {
    ...
    const u64 num_pages = ((vaddr + size - 1) >> UZUY_PAGEBITS) - (vaddr >> UZUY_PAGEBITS) + 1;
    for (u64 i = 0; i < num_pages; ++i, vaddr += UZUY_PAGESIZE) {
      const Common::PageType page_type {
        current_page_table -> pointers[vaddr >> UZUY_PAGEBITS].Type()
      };
      if (debug) {
        ...
        case Common::PageType::DebugMemory:
          ...
          break;
        case Common::PageType::Memory:
          current_page_table -> pointers[vaddr >> UZUY_PAGEBITS].Store(
        ...
        case Common::PageType::DebugMemory: {
          u8 *
            const pointer {
              GetPointerFromDebugMemory(vaddr & ~UZUY_PAGEMASK)
            };
          current_page_table -> pointers[vaddr >> UZUY_PAGEBITS].Store(
            reinterpret_cast < uintptr_t > (pointer) - (vaddr & ~UZUY_PAGEMASK),
            Common::PageType::Memory);
          break;
        }
        default:
    ...
    for (u64 i = 0; i < num_pages; ++i, vaddr += UZUY_PAGESIZE) {
      ...
      if (cached) {
        ...
        case Common::PageType::RasterizerCachedMemory: {
          u8 *
            const pointer {
              GetPointerFromRasterizerCachedMemory(vaddr & ~UZUY_PAGEMASK)
            };
          if (pointer == nullptr) {
  ...
  void MapPages(Common::PageTable & page_table, Common::ProcessAddress base_address, u64 size,
    ...
    LOG_DEBUG(HW_Memory, "Mapping {:016X} onto {:016X}-{:016X}", GetInteger(target),
      base * UZUY_PAGESIZE, (base + size) * UZUY_PAGESIZE);
    ...
    if (!target) {
      ASSERT_MSG(type != Common::PageType::Memory,
        "Mapping memory page without a pointer @ {:016x}", base * UZUY_PAGESIZE);
      ...
      while (base != end) {
        auto host_ptr =
          ...
          (base << UZUY_PAGEBITS);
        auto backing = GetInteger(target) - (base << UZUY_PAGEBITS);
        page_table.pointers[base].Store(host_ptr, type);
        page_table.backing_addr[base] = backing;
        page_table.blocks[base] = orig_base << UZUY_PAGEBITS;

        ASSERT_MSG(page_table.pointers[base].Pointer(),
          "memory mapping base yield a nullptr within the table");

        base += 1;
        target += UZUY_PAGESIZE;
      }
    }
  }
  ...
  [[nodiscard]] u8 * GetPointerImpl(u64 vaddr, auto on_unmapped, auto on_rasterizer) const {
    ...
    vaddr = vaddr & 0xffffffffffff ULL;
    ...
    const uintptr_t raw_pointer = current_page_table -> pointers[vaddr >> UZUY_PAGEBITS].Raw();
    ...
    case Common::PageType::RasterizerCachedMemory: {
      u8 *
        const host_ptr {
          GetPointerFromRasterizerCachedMemory(vaddr)
        };
      on_rasterizer();
      return host_ptr;
    }
   ...
   */
  template < typename T >
    void Write(Common::ProcessAddress vaddr,
      const T data) {
      u8 *
        const ptr = GetPointerImpl(
          GetInteger(vaddr),
  ...
  template < typename T >
    bool WriteExclusive(Common::ProcessAddress vaddr,
      const T data,
        const T expected) {
      u8 *
        const ptr = GetPointerImpl(
  ...
  bool WriteExclusive128(Common::ProcessAddress vaddr,
    const u128 data,
      const u128 expected) {
    u8 *
      const ptr = GetPointerImpl(
  ...
  void HandleRasterizerWrite(VAddr v_address, size_t size) {
    ...
    gpu_device_memory -> ApplyOpOnPointer(p, scratch_buffers[core], [ & ](DAddr address) {
      ...
      PAddr subaddress = address >> UZUY_PAGEBITS;
...
Memory::Memory(Core::System & system_): system {
  system_
} {
  Reset();
}
...
bool Memory::IsValidVirtualAddress(const Common::ProcessAddress vaddr) const {
  ...
  const size_t page = vaddr >> UZUY_PAGEBITS;
...
bool Memory::IsValidVirtualAddressRange(Common::ProcessAddress base, u64 size) const {
  ...
  Common::ProcessAddress page = Common::AlignDown(GetInteger(base), UZUY_PAGESIZE);

  for (; page < end; page += UZUY_PAGESIZE) {
...
const u8 * Memory::GetSpan(const VAddr src_addr,
  const std::size_t size) const {
  return impl -> GetSpan(src_addr, size);
}

u8 * Memory::GetSpan(const VAddr src_addr,
  const std::size_t size) {
  return impl -> GetSpan(src_addr, size);
}

bool Memory::WriteBlock(const Common::ProcessAddress dest_addr,
  const void * src_buffer,
    const std::size_t size) {
  return impl -> WriteBlock(dest_addr, src_buffer, size);
}

bool Memory::WriteBlockUnsafe(const Common::ProcessAddress dest_addr,
  const void * src_buffer,
    const std::size_t size) {
...
bool Memory::ZeroBlock(Common::ProcessAddress dest_addr,
  const std::size_t size) {
...
Result Memory::InvalidateDataCache(Common::ProcessAddress dest_addr,
  const std::size_t size) {
  return impl -> InvalidateDataCache(dest_addr, size);
}

Result Memory::StoreDataCache(Common::ProcessAddress dest_addr,
  const std::size_t size) {
  return impl -> StoreDataCache(dest_addr, size);
}

Result Memory::FlushDataCache(Common::ProcessAddress dest_addr,
  const std::size_t size) {
